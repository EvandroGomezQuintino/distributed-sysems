...
int main(int argc, char **argv) {
	...
	// calculate mean for this node
  	sub_mean = sum(sub_rand_nums, num_elements_per_proc);
	// reduce the sum
	...
	float differences = 0;
	differences = sumDifferences(sub_rand_nums, num_elements_per_proc, overall_average_recv);
	...
	if (rank == 0) {
		sub_means = (float *)malloc(sizeof(float) * size);
